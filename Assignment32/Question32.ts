...
let current_users = ['dany','morrison','jeny','sufyan'];
let new_users = ['jacky','wilson','john','SUFYAN'];

for (let i = 0; i < new_users.length; i++) {
    
    let new_username_lowercase = new_users[i].toLowerCase();
  
    let isUsernameTaken = current_users.some((username) =>  username.toLowerCase() === new_username_lowercase
    );
    ...
    else if(isUsernameTaken = current_users.some((username) =>  username.toLowerCase() === new_username_lowercase
    )){
    console.log(`Username '${new_users[i]}' should not be accepted`)
    }
    else {
